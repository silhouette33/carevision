        ...
        <div style={styles.container}>
            <div style={styles.card}>
                ...
                <div style={styles.tabs}>
                    <button style={{ ...styles.tab, ...(mode === 'login' ? styles.tabActive : {}) }} onClick={() => setMode('login')}>로그인</button>
                    <button style={{ ...styles.tab, ...(mode === 'register' ? styles.tabActive : {}) }} onClick={() => setMode('register')}>회원가입</button>
                </div>
                <form onSubmit={handleSubmit} style={styles.form}>
                    ...
                    <input style={styles.input} name="password" type="password" placeholder="비밀번호" value={form.password} onChange={handleChange} required />
                    {error && <p style={{ ...styles.message, color: error.includes('완료') ? '#16a34a' : '#dc2626' }}>{error}</p>}
