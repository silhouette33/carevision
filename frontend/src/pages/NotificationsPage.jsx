...
import { ChevronLeft, CheckCircle2, Bell, AlertTriangle, Pill, ShieldCheck, X, Video, Phone } from 'lucide-react';
...
export default function NotificationsPage({ onBack, onNavigate }) {
  ...
  const hasEmergency = notifications.some(n => n.type === 'FALL' && !n.isRead);
      ...
      <div className="min-h-screen bg-gray-50 max-w-[480px] mx-auto pb-24 relative font-sans">
        ...
        <div className="bg-white px-4 pt-6 pb-4 sticky top-0 z-30 shadow-sm border-b border-gray-100">
          <div className="flex items-center justify-between mb-4">
            <button onClick={onBack} className="p-2 -ml-2 bg-transparent border-none cursor-pointer text-gray-600">
              <ChevronLeft size={24} />
            </button>
            <h1 className="text-lg font-bold text-gray-900">알림 센터</h1>
            {unreadCount > 0 ? (
                <button
        ...
        <div className="p-4">
          ...
          {loading ? (
              <div className="text-center py-20 text-gray-400 text-sm font-bold animate-pulse">알림 불러오는 중...</div>
          ) : filtered.length === 0 ? (
              <div className="text-center py-24 bg-white rounded-[32px] border border-dashed border-gray-200">
                ...
                <p className="text-gray-400 font-bold text-sm">최근 도착한 알림이 없습니다.</p>
              </div>
          ) : (
              <div className="space-y-3">
                {filtered.map(n => {
                  const config = TYPE_CONFIG[n.type] || TYPE_CONFIG['NORMAL'];
                  return (
                      <div
                          key={n.id}
                          onClick={() => !n.isRead && handleMarkOne(n.id)}
                          className={`relative flex gap-4 p-4 rounded-[24px] bg-white border border-gray-100 transition-all shadow-sm ${
                              n.isRead ? 'opacity-60' : 'hover:shadow-md'
                          }`}
                      ...
                      <span className={`text-[10px] font-black px-2 py-0.5 rounded-md ${config.badge}`}>
                        {config.text}
                          ...
                          {n.patient.name} 환자
                            ...
                            {!n.isRead && <div className="w-2 h-2 bg-blue-600 rounded-full animate-pulse ml-auto" />}
                          </div>
                          <p className="text-[13px] font-bold text-gray-800 leading-snug mb-1 truncate-2">
                            {n.message}
                          </p>
                          <p className="text-[10px] text-gray-400 font-medium">
                            {new Date(n.sentAt).toLocaleString('ko-KR', { hour12: true, hour: '2-digit', minute: '2-digit', month: 'short', day: 'numeric' })}
                          </p>
                        </div>
                      </div>
                  );
              ...
              <div className="bg-red-600 rounded-[28px] p-5 shadow-2xl shadow-red-200 border border-red-500">
                <div className="flex justify-between items-start mb-3">
                  <div className="flex items-center gap-2 text-white">
                    ...
                    <div>
                      <h4 className="text-[14px] font-black">긴급 상황 발생</h4>
                      <p className="text-[11px] text-red-100 opacity-90 font-bold">낙상이 감지되었습니다. 즉시 확인하세요!</p>
                    </div>
                  </div>
                  <button
